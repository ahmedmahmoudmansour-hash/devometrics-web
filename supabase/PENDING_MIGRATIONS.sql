...
-- pattern as the existing theme column) PLUS 0087 (Knowledge Hub exam
-- attempt limits: an admin-configurable max_attempts cap per exam document,
-- and a 2-hour retry cooldown replacing the old 60-second anti-double-click
-- one, so a failed attempt actually encourages re-reading the material
-- before retrying). Every statement is idempotent
...
-- 0079 last defined; 0084, 0085, 0086, and 0087 are each independent of
-- everything before them and safe to run standalone, though 0085 and 0087
-- depend on 0084's tables/functions existing first) — paste and run the
-- whole thing as one block.
--
-- NOTE: 0076-0086 are already applied to production (confirmed
-- 2026-07-26) — you only need to run the standalone
-- migrations/0087_knowledge_hub_exam_attempts.sql file on its own.
--
