...
export async function generateCandidateRanking(postingId: string): Promise<{ error: string } | { success: true }> {
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 3000,
      system:
        `LANGUAGE: Write every free-text field in ${locale === "ar" ? "Modern Standard Arabic (Fusha)" : "English"}, regardless of what language the candidate data below happens to be recorded in.\n\n` +
