...
- Corporate/Enterprise Platform — self-serve, no sales call required: any team member signs up, then sets up a company workspace in under a minute. Real multi-tenant, each company's data fully isolated from every other company. The admin adds employee emails one at a time or via a bulk CSV/Excel import — those people are automatically attached to the company the moment they sign up with that exact email address (no shared join code — invites are per-email, and each invited person gets an email with a signup link). The admin's HR dashboard shows a workforce skill inventory, a talent heatmap across the team's competencies, a leadership-readiness signal (explicitly labeled as a directional signal, not a formal succession plan, since that needs org-chart/role-criticality data this product doesn't capture), a custom competency framework builder, anonymous culture/pulse surveys, and executive-level assessments.
