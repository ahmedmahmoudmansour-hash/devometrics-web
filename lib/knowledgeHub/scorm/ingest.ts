...
type ExtractedFile = { relativePath: string; mimeType: string; data: Buffer };

// Reads every real (non-directory) entry in the zip into memory, enforcing
// every hard limit against the archive's own declared central-directory
// metadata BEFORE that entry's bytes are decompressed — yauzl parses the
// central directory up front (sizes are known without inflating anything),
// so a check against entry.uncompressedSize/compressedSize here genuinely
// runs pre-decompression, not as an after-the-fact cleanup. Path traversal
// (".."/absolute paths/backslashes) is rejected unconditionally by yauzl
// itself during parsing (validateFileName, always-on whenever
// decodeStrings is true) — that surfaces as a thrown error from
// zipfile.eachEntry() below, not as a check this function hand-rolls.
async function extractScormZip(zipBuffer: Buffer): Promise<{ error: string } | { files: ExtractedFile[] }> {
  ...
  try {
    for await (const entry of zipfile.eachEntry()) {
      if (Date.now() - startedAt > SCORM_PROCESSING_BUDGET_MS) {
        return { error: "This package is too large or complex to process. Try a smaller SCORM package." };
      }
      ...
      if (isSymlink) {
        return { error: `Package rejected: "${entry.fileName}" is a symlink, which isn't allowed in an uploaded package.` };
      }

      fileCount += 1;
      if (fileCount > SCORM_MAX_FILE_COUNT) {
        return { error: `Package rejected: more than ${SCORM_MAX_FILE_COUNT} files.` };
      }

      if (entry.uncompressedSize > SCORM_MAX_FILE_BYTES) {
        return { error: `Package rejected: "${entry.fileName}" is larger than ${SCORM_MAX_FILE_BYTES / (1024 * 1024)}MB uncompressed.` };
      }
      ...
      if (entry.compressedSize > 0 && entry.uncompressedSize / entry.compressedSize > SCORM_MAX_COMPRESSION_RATIO) {
        return { error: `Package rejected: "${entry.fileName}" has an implausible compression ratio.` };
      }

      totalUncompressed += entry.uncompressedSize;
      if (totalUncompressed > SCORM_MAX_UNCOMPRESSED_BYTES) {
        return { error: `Package rejected: total uncompressed size exceeds ${SCORM_MAX_UNCOMPRESSED_BYTES / (1024 * 1024)}MB.` };
      }
      ...
      files.push({ relativePath: entry.fileName, mimeType: guessMimeType(entry.fileName), data: Buffer.concat(chunks) });
    }
  } catch (err) {
    return { error: err instanceof Error ? err.message : "Could not process this package." };
  }

  if (files.length === 0) return { error: "This package is empty." };
  return { files };
}
...
// after the upload itself succeeded, and on failure nothing is left
// registered (the orphaned raw zip in Storage is harmless clutter, not a
// half-created content row someone could stumble into).
export async function validateAndRegisterScormPackage(input: {
  ...
  const extracted = await extractScormZip(Buffer.from(await zipBlob.arrayBuffer()));
  if ("error" in extracted) return { error: extracted.error };

  const manifestFile = extracted.files.find((f) => f.relativePath.toLowerCase() === "imsmanifest.xml");
  if (!manifestFile) return { error: "This package has no imsmanifest.xml at its root — not a valid SCORM package." };

  const manifestInfo = parseScormManifest(manifestFile.data.toString("utf8"));
  if ("error" in manifestInfo) return { error: manifestInfo.error };

  if (!extracted.files.some((f) => f.relativePath === manifestInfo.launchPath)) {
    return { error: `The manifest's launch file ("${manifestInfo.launchPath}") isn't in the package.` };
  }

  // Re-upload every unpacked file individually — playback afterward is then
  // just plain static files served through the same-origin scorm proxy
  // route (app/api/knowledge-hub/scorm/.../route.ts), no runtime
  // unzip-on-request infrastructure needed. Path convention matches the
  // plan: {organization_id}/{content_id}/scorm/{relative_path}.
  const scormPrefix = `${orgId}/${input.contentId}/scorm`;
  for (const file of extracted.files) {
    const { error: uploadError } = await supabase.storage
      .from(KNOWLEDGE_HUB_BUCKET)
      .upload(`${scormPrefix}/${file.relativePath}`, file.data, { contentType: file.mimeType, upsert: true });
    if (uploadError) {
      return { error: `Could not upload "${file.relativePath}" — try uploading the package again.` };
    }
  }

  const totalBytes = extracted.files.reduce((sum, f) => sum + f.data.length, 0);
  ...
  const { error: contentError } = await supabase.from("knowledge_hub_content").insert({
    ...
    file_name: manifestInfo.launchPath,
    file_size_bytes: Math.max(1, totalBytes),
    ...
    created_by: user.id,
    content_type: "scorm",
    scorm_version: manifestInfo.version,
    scorm_launch_path: manifestInfo.launchPath,
  });
  if (contentError) {
    return { error: "Package processed, but could not save it — the database may need migration 0149 run first." };
  }
